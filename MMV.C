...
static void init()
{
        ...
        DosQCurDisk((PUSHORT) &curdisk, &drivemap);
...
#ifdef MSDOS
        if (*p == '.' || (!matchall && ffrom->fi_attrib & (FILE_HIDDEN | FILE_SYSTEM)))
	...
	int *pflags;
{
	...
	*pflags = 0;
	if (
#ifdef MSDOS
                (ffrom->fi_attrib & FILE_DIRECTORY) &&
#else
	...
	int which;
{
	...
	if (patch.ph_safeid) {
		...
		strcpy(pathend + STRLEN(IDF), "*");
                hdir = count = 1;
                if (DosFindFirst(p, &hdir, 0, &de, sizeof(de), &count, 0L)) {
			if ((d = ndirs) == 1000) {
			...
			strcpy(pathend, "*.*");
                        hdir = count = 1;
                        if (DosFindFirst(p, &hdir, FILE_DIRECTORY | FILE_SYSTEM | FILE_HIDDEN, &de, sizeof(de), &count, 0L))
				h->h_di = dadd(v, d);
			else
		...
		*pathend = '\0';
		...
		strcpy(pathend, "*.*");
                hdir = count = 1;
                firstfound = !DosFindFirst(p, &hdir, FILE_DIRECTORY | FILE_SYSTEM | FILE_HIDDEN, &de, sizeof(de), &count, 0L);
		*pathend = '\0';
		if (firstfound) {
			v = DRIVENO(&de);
			d = CLUSTNO(&de);
	...
	REP *p;
{
	...
	if (
#ifdef MSDOS
                p->r_fdel->fi_attrib & FILE_READONLY
#else
#ifndef SYSV
	...
	long len;
{
	...
	if (!(op & (APPEND | OVERWRITE)))
		if (
#ifdef MSDOS
                        DosQFileInfo(f, 1, &fs, sizeof(fs)) ||
                        DosSetFileInfo(t, 1, (PBYTE) &fs, sizeof(fs)) ||
                        DosSetFileMode(fullrep, ff->fi_attrib, 0L)
#else
			stat(pathbuf, &fstat) ||
			(
#ifdef SYSV
	...
	FILEINFO *f;
{
	...
	int a;
        ...
        if (((a = f->fi_attrib) & FILE_READONLY) && DosSetFileMode(n, a & ~FILE_READONLY, 0L)) {
